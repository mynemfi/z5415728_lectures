...
def get_tics(pth):
    ...
    with open(pth, mode = 'r') as fobj:
        dic = {}
        for line in fobj:
            exchange, tic = line.strip().lower().replace('"','').split('=')
            dic[tic] = exchange
        return dic
...
def read_dat(tic):
    ...
    # <COMPLETE THIS PART>
    DATPATH = os.path.join(DATDIR, f'{tic}_prc.dat')
    with open(DATPATH, mode='r') as fobj:
        lis = []
        for line in fobj:
    ...
    if tickers_lst is not None:
        if tickers_lst == []:
            ...
            for item in tickers_lst:
                if item in list(tic_exchange_dic.keys()):
                    continue
                else:
                    raise Exception(f'{item} is not a key of tic_exchange_dic')
        ...
        if col_lst == []:
            ...
            for item in col_lst:
                if item in COLUMNS:
                    ...
                    raise Exception(f'{item} is not found in `COLUMNS`')
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for tic in tickers_lst:
        ...
        for line in read_dat(tic):
            i = {}
            for col in col_lst:
                i[col] = line_to_dict(line)[col]
            data.append(i)
        dic[tic] = {'exchange': tic_exchange_dic[tic], 'data': data}
