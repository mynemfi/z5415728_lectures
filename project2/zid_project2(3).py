...
def mk_prc_df(tickers, prc_col='adj_close'):
    ...
    **not** necessarily represent the actual contents of the data frame you
    ...
    for ticker in tickers:
        # 使用 read_prc_csv 读取每个股票的数据
        df = read_prc_csv(ticker)

        # 检查 prc_col 是否在 df 中
        if prc_col not in df.columns:
            raise ValueError(f"Column {prc_col} not found in {ticker} data")

        # 将股票价格列添加到结果 DataFrame 中
        combined_df[ticker.lower()] = df[prc_col]

    # 使用日期作为索引，重建 DataFrame
...
def mk_ret_df(prc_df):
    ...
    # 读取市场收益率数据
    market_df = pd.read_csv(cfg.FF_CSV, parse_dates=['Date'], index_col='Date')
    market_ret = market_df['mkt']

    # 将市场收益率合并到个股收益率数据框中
    ret_df = ret_df.join(market_ret, how='left')
...
def mk_aret_df(ret_df):
    ...
    # <COMPLETE THIS PART>
    # 提取市场收益
    market_ret = ret_df['mkt']

    # 计算异常收益
    # 使用 .sub() 函数从每个股票的收益中减去市场收益
    abnormal_returns = ret_df.sub(market_ret, axis=0)

    # 移除市场收益列 'mkt'
    abnormal_returns.drop('mkt', axis=1, inplace=True)
...
def get_avg(df, col, year):
    ...
    # 计算并返回该年份指定列的平均值
    return year_df[col].mean(axis=0, skipna = True)
...
def get_ew_rets(df, tickers):
    ...
    # # 筛选出指定股票的数据
    # selected_df = df[tickers]
    # # 删除缺失行
    # # 计算等权重平均收益率
    # ew_returns = selected_df.mean(axis=1,skipna=True)
    # return ew_returns
    # df按照日期排序
    df.sort_index(inplace=True)

    # 筛选出指定股票的数据，并创建副本以避免 SettingWithCopyWarning
    selected_df = df[tickers].copy()

    # 删除所有值都是NaN的行
    selected_df.dropna(axis=0, how='all', inplace=True)

    # 计算等权重平均收益率
    ew_returns = selected_df.mean(axis=1)
...
def get_ann_ret(ser, start, end):
    ...
    # 筛选指定日期范围的数据
    filtered_ser = ser[start:end]

    # 删除缺失值
    filtered_ser.dropna(inplace=True)

    # 计算总体收益率
    tot_ret = (1 + filtered_ser).prod()

    # 计算非缺失回报的天数
    N = filtered_ser.count()

    # 计算并返回年化收益率,小数点后四位
    return str((tot_ret ** (252 / N)) - 1).split(".")[0] + "." + str((tot_ret ** (252 / N)) - 1).split(".")[1][:4]
    # 不保留四位
